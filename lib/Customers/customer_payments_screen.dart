  ...
  Future<void> _fetchPayments() async {
    ...
    try {
      final params = <String, String>{
        'page': '1',
        'limit': '100',
        if (_filterMethod != 'all') 'payment_method': _filterMethod,
        if (_dateRange != null) 'from_date': _dateRange!.start.toIso8601String(),
        if (_dateRange != null) 'to_date': _dateRange!.end.toIso8601String(),
      };
      ...
      // ── Primary: dedicated payments endpoint ──────────────────────────────
      final primaryUri = Uri.parse(
        '${ApiConfig.baseUrl}/customer-ledger/${widget.customer.id}/payments',
      ).replace(queryParameters: params);

      print('Fetching payments from: $primaryUri');

      var response = await http.get(primaryUri, headers: headers);

      // ── Fallback: ledger endpoint filtered by transaction_type ────────────
      if (response.statusCode == 404) {
        print('Payments endpoint not found, falling back to ledger endpoint...');

        final fallbackUri = Uri.parse(
          '${ApiConfig.baseUrl}/customer-ledger/${widget.customer.id}',
        ...
        print('Fallback URI: $fallbackUri');
        response = await http.get(fallbackUri, headers: headers);
      }

      // ── Guard: HTML error page (misconfigured server / wrong route) ────────
      if (response.headers['content-type']?.contains('text/html') ?? false) {
        throw Exception('Server returned HTML — check backend route configuration.');
      }
      ...
      if (response.statusCode != 200 || data['success'] != true) {
        setState(() {
          _error = data['message'] ?? 'Failed to load payments (${response.statusCode})';
          _isLoading = false;
        });
        return;
      }

      // ── Extract payment list (handle both response shapes) ─────────────────
      List<Map<String, dynamic>> paymentList = [];

      final responseData = data['data'];
      if (responseData != null) {
        if (responseData['payments'] != null) {
          // Shape A — dedicated payments endpoint
          paymentList = (responseData['payments'] as List)
              .map<Map<String, dynamic>>((e) => Map<String, dynamic>.from(e))
              .toList();
        } else if (responseData['entries'] != null) {
          // Shape B — ledger endpoint (extra safety: keep only payment entries)
          paymentList = (responseData['entries'] as List)
              .where((e) => e['transaction_type'] == 'payment')
              .map<Map<String, dynamic>>((e) => Map<String, dynamic>.from(e))
              .toList();
        } else if (responseData is List) {
          // Shape C — bare list
          paymentList = (responseData as List)
              .map<Map<String, dynamic>>((e) => Map<String, dynamic>.from(e))
              .toList();
        }
      }

      // ── Client-side method filter (safety net if backend skipped it) ───────
      if (_filterMethod != 'all') {
        paymentList = paymentList.where((p) {
          final method = p['payment_method']?.toString().toLowerCase() ?? '';
          // Also check description as fallback (matches legacy entries)
          final desc = p['description']?.toString().toLowerCase() ?? '';
          return method == _filterMethod ||
              (method.isEmpty && desc.contains(_filterMethod));
        }).toList();
      }

      // ── Compute total (credit column = amount received) ────────────────────
      final double total = paymentList.fold(0.0, (sum, p) {
        final amount = double.tryParse(
          (p['credit'] ?? p['amount'] ?? '0').toString(),
        ) ??
            0.0;
        return sum + amount;
      });
            ...
            return Padding(
              ...
              child: GestureDetector(
                onTap: () async {
                  ...
                  await _fetchPayments();  // await after setState completes
                },
